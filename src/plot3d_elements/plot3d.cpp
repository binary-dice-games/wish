...
void register_plot3d() {
  ...
  // Plot3D — ImPlot3D window container. Children should be Plot3DItem elements.
  {
    ...
    proto->addField("title"_key, field{std::string{"##plot3d"},
      attr<DisplayName>("Title"),
      attr<Description>(
          "Plot title shown at the top; prefix ## to hide the title bar."),
      attr<Category>("Content")});
    proto->addField("x_label"_key, field{std::string{},
      attr<DisplayName>("X Label"),
      attr<Description>("X axis label; empty string = no label."),
      attr<Category>("Axes")});
    proto->addField("y_label"_key, field{std::string{},
      attr<DisplayName>("Y Label"),
      attr<Description>("Y axis label; empty string = no label."),
      attr<Category>("Axes")});
    proto->addField("z_label"_key, field{std::string{},
    ...
    proto->addField("width"_key, field{float{-1.0f},
      ...
      attr<Description>(
          "Plot width in pixels; -1 fills the available horizontal space."),
      attr<Category>("Layout"),
      attr<Range>(-1.0, 8192.0)});
    proto->addField("height"_key, field{float{400.0f},
