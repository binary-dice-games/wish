...
static void merge_fields(const bison::dynamic& src, bison::dynamic& dst) {
  ...
  const_cast<bison::dynamic&>(src).forEach(
      [&dst](key_t k, const field& f) {
        if (f.is<std::string>())  dst[k] = f.as<std::string>();
        else if (f.is<float>())   dst[k] = f.as<float>();
        else if (f.is<int32_t>()) dst[k] = f.as<int32_t>();
        else if (f.is<bool>())    dst[k] = f.as<bool>();
      });
}
...
void style_service::set_preset(const std::string& name) {
  if (name != "dark" && name != "light" && name != "classic") {
    throw std::runtime_error(
        "wish::style_service: unknown preset '" + name +
        "'; expected 'dark', 'light', or 'classic'");
  }
