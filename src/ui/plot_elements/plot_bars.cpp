...
static void add_bar_fields(dynamic_ptr& proto) {
  proto->addField(
      ...
      field{
          ...
          attr<Description>("Array of bar X positions.  "
...
void register_plot_bars() {
  // PlotBars — vertical bar chart.
  {
    ...
    (*proto)[dynamic::CLASS].addAttribute(
        attr<Description>("Draws vertical bars at each xs[i] with height ys[i].  "
  ...
  // PlotBarsH — horizontal bar chart.
  {
    ...
    (*proto)[dynamic::CLASS].addAttribute(
        attr<Description>("Draws horizontal bars at each ys[i] with length xs[i].  "
