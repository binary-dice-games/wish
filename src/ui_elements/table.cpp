...
void register_table() {
  ...
  // TableHeadersRow if headers == true, then renders the remaining children.
  {
    ...
    proto->addField("id"_key, field{std::string{"##table"},
      attr<DisplayName>("ID"),
      attr<Description>("ImGui string identifier for this table."),
      attr<Category>("Behavior")});
    proto->addField("columns"_key, field{int32_t{1},
  ...
  // Must be a direct child of Table and precede any TableRow children.
  {
    ...
    proto->addField("flags"_key, field{int32_t{0},
      ...
      attr<Category>("Behavior")});
    proto->addField("init_width"_key, field{float{0.0f},
  ...
  // Each direct child of a TableRow is rendered into the next table column.
  {
    ...
    proto->addField("flags"_key, field{int32_t{0},
      ...
      attr<Category>("Behavior")});
    proto->addField("min_height"_key, field{float{0.0f},
