...
void register_input_number() {
  // InputInt — integer input with +/- step buttons.
  {
    ...
    proto->addField("value"_key, field{int32_t{0},
      ...
      attr<Category>("State")});
    proto->addField("step"_key, field{int32_t{1},
      attr<DisplayName>("Step"),
      attr<Description>("Amount added/subtracted by the +/- buttons."),
      attr<Category>("Behavior")});
    proto->addField("step_fast"_key, field{int32_t{100},
  ...
  // InputFloat — float input with optional step buttons.
  {
    ...
    proto->addField("value"_key, field{float{0.0f},
      ...
      attr<Category>("State")});
    proto->addField("step"_key, field{float{0.0f},
      attr<DisplayName>("Step"),
      attr<Description>("Amount added/subtracted by the +/- buttons. 0 hides the buttons."),
      attr<Category>("Behavior")});
    proto->addField("step_fast"_key, field{float{0.0f},
      attr<DisplayName>("Step Fast"),
