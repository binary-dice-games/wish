...
void register_text_editor() {
  ...
  proto->addField("file_path"_key, field{std::string{""},
    attr<DisplayName>("File Path"),
    attr<Description>(
        "Path to the file to edit. Relative paths are sandboxed inside the "
        "session resource directory. Absolute paths require the server to be "
        "configured with set_allow_absolute_paths(true)."),
    attr<Category>("Content"),
    attr<Required>()});
  proto->addField("language"_key, field{std::string{"none"},
    attr<DisplayName>("Language"),
    attr<Description>(
        "Syntax highlighting language. Supported values: "
        "\"cpp\", \"c\", \"cs\", \"glsl\", \"hlsl\", \"lua\", "
        "\"python\", \"sql\", \"json\", \"markdown\", \"angelscript\", "
        "\"none\"."),
    attr<Category>("Content")});
  proto->addField("read_only"_key, field{bool{false},
  ...
  proto->addField("width"_key, field{int32_t{0},
    ...
    attr<Range>(0, 8192),
    attr<Step>(1)});
  proto->addField("height"_key, field{int32_t{400},
