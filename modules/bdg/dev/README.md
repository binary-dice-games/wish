...
| [dbg](dbg/README.md) | Source-level debugger GUI for a local process: attach by PID, breakpoints, stepping, a simple-expression Watch window, and a debug/exception Output log. Server owns the UI; the client backend is the Win32 debug API / DbgHelp on Windows, or a child `gdb`/`lldb-mi` driven over GDB/MI on Linux/macOS. |
