...
| [dbg](dbg/README.md) | Source-level debugger GUI for a local process: attach by PID, breakpoints, stepping, a simple-expression Watch window, and a debug/exception Output log. Server owns the UI, client owns the Win32 debug API / DbgHelp backend. Windows-only. |
