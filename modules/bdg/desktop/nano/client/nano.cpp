...
/// @file nano.cpp
/// @brief Client-side runner for the nano embedded app.
///
/// The Nano form (server-side) never touches the client's local
...
// name and register it as a new Nano tab. Shared by the FileDialog-driven
// Open/New flows and by opening a file passed on the command line.
void upload_and_open(
    wish_app_host& s,
    const std::shared_ptr<rmi::proxy::dynamic>& nano,
  ...
  nano->call("open_file"_key, std::move(args)).get();
}
...
void browse_and_open(
    wish_app_host& s,
    std::shared_ptr<rmi::proxy::dynamic> nano,
  ...
  dlg->onEvent("on_open"_key, [&s, nano, files, cur_dir, create_if_missing](dynamic payload) {
...
void run_nano(wish_app_host& s) {
  auto nano = std::make_shared<rmi::proxy::dynamic>(s.instantiate("wish"_key, "Nano"_key).get());
  ...
  nano->onEvent("on_request_open"_key, [&s, nano, files](dynamic) {
    browse_and_open(s, nano, files, "Open File", "Open", /*create_if_missing=*/false);
  });

  // "New" clicked: same handshake, but the chosen path need not exist yet.
  nano->onEvent("on_request_new"_key, [&s, nano, files](dynamic) {
    browse_and_open(s, nano, files, "New File", "New", /*create_if_missing=*/true);
  });

  // A tab (or the whole window) closed: download this file one last time
  // and forget our local bookkeeping for it.
  nano->onEvent("on_file_closed"_key, [&s, files](dynamic payload) {
  ...
  nano->onEvent("closed"_key, [&s](dynamic) { s.signal_done(); });
  // Nothing else to keep alive: `nano`'s shared_ptr lives on inside the
  ...
  if (const auto& app_args = s.app_args(); !app_args.empty()) {
    ...
    if (fs::exists(local_path))
      ...
      std::cerr << "[nano] no such file: " << local_path.string() << '\n';
  }
...
const nano_app_registrar nano_app_registrar_instance;
} // namespace
