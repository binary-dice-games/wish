...
/// @file zip_tool.hpp
/// @brief Server-side ZipTool form: UI only, no local file access.
#pragma once
...
///
/// Unlike FileExplorer's sandbox (right) panel or ZipTool's own first draft,
...
/// directly. It follows exactly the handshake FileExplorer's local (left)
...
/// `modules/bdg/desktop/zip_tool/client/zip_tool.cpp` for the reference
...
class zip_tool : public form {
 public:
  explicit zip_tool(bison::dynamic&& base);
  ...
  /// FileExplorer's `update_local_listing()`).
 ...
 private:
  ...
  /// do -- mirrors file_explorer.cpp's `pending_transfer`.
  ...
  /// first -- see file_explorer.cpp's sort_entries() for the reference
  /// this mirrors.
...
/// @brief Register ZipTool in the "wish" bison namespace.
