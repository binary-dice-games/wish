...
/// @file zip_tool.cpp
/// @brief Implementation of the ZipTool form.
///
...
/// modules/bdg/desktop/zip_tool/client/zip_tool.cpp instead. See
/// zip_tool.hpp's class doc comment for the full client/server handshake.
#include "zip_tool.hpp"
...
// file_explorer.cpp's server and client copies of it are duplicated -- see
...
// ── UI layout ─────────────────────────────────────────────────────────────────
//
// Single-panel client-machine browser -- mirrors file_explorer.cpp's local
...
// file_explorer's tables. InputText EnterReturnsTrue=32 so the path bar only
...
// exists (per the cached listing) -- mirrors file_explorer.cpp's
...
// omission file_explorer.cpp's own comment calls out for FileDialog's table.
...
zip_tool::zip_tool(dynamic&& base) : form(std::move(base)) {}

void zip_tool::on_init() {
  internal_root_key_ = next_available_key("__ziptool_");

  auto tree = import_json(kLayout);

  auto* title_f = findField<std::string>("title"_key);
  (*tree[""])["title"_key] = title_f ? *title_f : std::string{"Zip Tool"};
...
void zip_tool::fill_table(const std::vector<file_row>& entries, int32_t selected_index) {
  if (!file_table_ptr_)
    return;
...
void zip_tool::fill_contents_table(const ui_element_ptr& table, const std::vector<archive_entry>& entries) const {
  if (!table)
    return;
...
void zip_tool::sort_entries(std::vector<file_row>& entries, int32_t sort_column_id, bool ascending) const {
...
void zip_tool::on_table_sorted(const dynamic& payload) {
...
void zip_tool::set_status(const std::string& message) {
  (*this)["status"_key] = message;
  if (status_label_ptr_)
    status_label_ptr_["text"_key] = message;
}

bool zip_tool::is_zip_name(const std::string& name) const {
  if (name.size() < 4)
    return false;
  std::string ext = name.substr(name.size() - 4);
  for (auto& c : ext)
...
std::string zip_tool::cached_entry_type(const std::string& name) const {
  for (auto& e : entries_)
    if (e.name == name)
      return e.type;
  return {};
...
dynamic zip_tool::do_update_listing(const dynamic& args) {
...
dynamic zip_tool::do_show_contents(const dynamic& args) {
...
dynamic zip_tool::on_set(const dynamic& patch) {
...
void zip_tool::show_prompt(pending_action action, const std::string& source_name, const std::string& default_value) {
  // Called from on_event(), outside dispatch -- mirrors
  // file_explorer.cpp's show_overwrite_confirm() for the same reason: sess()
  ...
  context& s = *lock;
  for (int i = 0;; ++i) {
    std::string candidate = "__ziptool_prompt_" + std::to_string(i);
...
void zip_tool::request_close_prompt() {
  request_close_at(prompt_root_key_);
}

void zip_tool::remove_prompt_objects() {
  remove_objects_at(prompt_root_key_);
  prompt_root_key_.clear();
}

void zip_tool::emit_action_request(
...
void zip_tool::on_prompt_confirmed() {
...
void zip_tool::show_overwrite_confirm(const std::string& message) {
  ...
  context& s = *lock;
  for (int i = 0;; ++i) {
    std::string candidate = "__ziptool_confirm_" + std::to_string(i);
    if (s.top_level_objects.find(key_t{candidate}) == s.top_level_objects.end()) {
...
void zip_tool::request_close_confirm() {
  request_close_at(confirm_root_key_);
}

void zip_tool::remove_confirm_objects() {
...
void zip_tool::show_contents_dialog(const std::string& zip_name, const std::vector<archive_entry>& entries) {
  ...
  context& s = sess();
  for (int i = 0;; ++i) {
    std::string candidate = "__ziptool_contents_" + std::to_string(i);
    if (s.top_level_objects.find(key_t{candidate}) == s.top_level_objects.end()) {
...
void zip_tool::request_close_contents() {
  request_close_at(contents_root_key_);
}

void zip_tool::remove_contents_objects() {
...
void zip_tool::on_event(key_t id, key_t event, const dynamic& payload) {
...
void register_zip_tool() {
  auto proto = dynamic_ptr{"ZipTool"_key, {}};
  ...
  proto->addMethod(
      "update_listing"_key, bison::method{[](dynamic& self, const dynamic& args) -> dynamic {
        return static_cast<zip_tool&>(self).do_update_listing(args);
      }});
  proto->addMethod(
      "show_contents"_key, bison::method{[](dynamic& self, const dynamic& args) -> dynamic {
        return static_cast<zip_tool&>(self).do_show_contents(args);
      }});
  proto->addMethod("__setter"_key, bison::method{[](dynamic& s, const dynamic& p) -> dynamic {
                     return static_cast<zip_tool&>(s).on_set(p);
                   }});

  (*proto)[dynamic::CLASS].addAttribute(attr<DisplayName>("ZipTool"));
  ...
  dynamic::addClass(
      "wish"_key, std::move(proto), key_t{0U}, dynamic::make_factory<zip_tool>("wish"_key, "ZipTool"_key));
}
