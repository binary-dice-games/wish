...
/// @file zip_tool.cpp
/// @brief Client-side runner for the ZipTool embedded app.
///
/// The ZipTool form (server-side) owns and renders the browser, but has no
...
#include "modules/bdg/desktop/zip_tool/client/zip_tool.hpp"
...
// the shape ZipTool::do_update_listing() expects:
...
void compress(const fs::path& dir, const std::string& source_name, const std::string& archive_name) {
  ...
  // ever emitting the request (see zip_tool.cpp (server)'s
...
void run_zip_tool(wish_app_host& s) {
  auto tool = std::make_shared<rmi::proxy::dynamic>(s.instantiate("wish"_key, "ZipTool"_key).get());
  ...
  // bar's value was changed) -- mirrors file_explorer's on_local_navigate.
  ...
  // FileExplorer's own root-on-open behavior for its local panel.
  report_listing(tool, cur_dir);
...
namespace {
struct zip_tool_app_registrar {
  zip_tool_app_registrar() {
    register_app({
        .name = "zip_tool",
        .organization = WISH_MODULE_BDG_DESKTOP_ZIP_TOOL_ORGANIZATION,
        .collection = WISH_MODULE_BDG_DESKTOP_ZIP_TOOL_COLLECTION,
...
const zip_tool_app_registrar zip_tool_app_registrar_instance;
} // namespace
