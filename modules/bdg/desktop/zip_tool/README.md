...
- **server/**: `ZipTool` form (`register_zip_tool()`), a `bdg::wish::form`
  ...
  probe.
- **client/**: `run_zip_tool(wish_app_host&)`, self-registered as the
  `"zip_tool"` embedded app — instantiates the form, enumerates the local
...
Mirrors `file_explorer`'s client/server split for its local (left) panel:
