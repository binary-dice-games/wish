...
- **server/**: `LogTail` form (`register_log_tail()`) -- parses/classifies
  ...
  `push_lines` gives it.
- **client/**: `run_log_tail(wish_app_host&)`, self-registered as the
  `"log_tail"` embedded app -- owns actual `tail`-style file reading (and,
...
- **resources/**: `patterns.json` -- the line-parsing rule set: regexes that
  ...
  Lands per-session at `res/bdg/desktop/log_tail/patterns.json` (see
