...
/// @file log_tail.cpp
/// @brief Implementation of the LogTail form.
  ...
  "children": {
    "vbox": {
      ...
      "children": {
        ...
        "tab_bar": {
          "type": "TabBar", "id": "##log_tail_tabs",
          "children": {
            "tab_all": {
              "type": "TabItem", "label": "All", "closable": false,
              "children": {
                "table_all": {
                  "type": "Table", "id": "##log_tail_all", "columns": 5, "headers": true,
...
log_tail::log_tail(dynamic&& base) : form(std::move(base)) {}

void log_tail::on_init() {
  // See form::internal_root_key_'s doc comment: ordinally-assigned, not pointer-derived.
  internal_root_key_ = next_available_key("__log_tail_");

  auto tree = import_json(kLayout);

  auto* title_f = findField<std::string>("title"_key);
  (*tree[""])["title"_key] = title_f ? *title_f : std::string{"Log Tail"};
  ...
  // res/bdg/desktop/log_tail/patterns.json. Never fails hard -- on any
  ...
  auto resolved = file_service::resolve_path(
      "res/bdg/desktop/log_tail/patterns.json", sess().resource_dir, sess().allow_absolute_paths);
  if (!resolved.empty()) {
...
dynamic log_tail::do_push_lines(const dynamic& args) {
...
dynamic log_tail::do_set_filter(const dynamic& args) {
  std::string pattern;
...
void log_tail::ingest_line(const std::string& raw, const std::string& source) {
...
bool log_tail::passes_filter(const std::string& raw) const {
  if (!filter_regex_)
    return true;
  return std::regex_search(raw, *filter_regex_);
}

void log_tail::append_row(log_table_state& state, const parsed_log_line& pl) {
  if (!state.table_ptr)
    return;
...
ui_element_ptr log_tail::build_log_table(log_table_state& state) {
  ui_element_ptr table{dynamic::instantiate("wish"_key, "Table"_key)};
  table["id"_key] = "##log_tail_tbl_" + std::to_string(next_table_seq_++);
...
log_tail::tag_tab_state& log_tail::ensure_tag_tab(const std::string& tag) {
...
void log_tail::close_tag_tab(const std::string& tag) {
...
void log_tail::apply_filter_from_input() {
  if (!filter_input_ptr_)
    return;
  dynamic args;
...
void log_tail::on_event(key_t id, key_t event, const dynamic& /*payload*/) {
...
void register_log_tail() {
  auto proto = dynamic_ptr{"LogTail"_key, {}};
  ...
  proto->addMethod(
      "push_lines"_key, bison::method{[](dynamic& self, const dynamic& args) -> dynamic {
        return static_cast<log_tail&>(self).do_push_lines(args);
      }});

  proto->addMethod(
      "set_filter"_key, bison::method{[](dynamic& self, const dynamic& args) -> dynamic {
        return static_cast<log_tail&>(self).do_set_filter(args);
      }});

  (*proto)[dynamic::CLASS].addAttribute(attr<DisplayName>("LogTail"));
  ...
  dynamic::addClass(
      "wish"_key, std::move(proto), key_t{0U}, dynamic::make_factory<log_tail>("wish"_key, "LogTail"_key));
}
