...
void log_line_parser::load_from_json(const std::string& json_text) {
  ...
  try {
    ...
    std::cerr << "[log_tail] failed to parse patterns.json: " << e.what() << " -- keeping previous rules\n";
    return;
  }
  ...
  if (root.contains("line_formats") && root["line_formats"].is_array()) {
    for (auto& item : root["line_formats"]) {
      ...
      try {
        ...
        std::cerr << "[log_tail] skipping line_formats rule \"" << rule.name << "\": invalid regex: " << e.what()
                   << '\n';
        continue;
      }
  ...
  if (root.contains("level_rules") && root["level_rules"].is_array()) {
    for (auto& item : root["level_rules"]) {
      ...
      try {
        ...
        std::cerr << "[log_tail] skipping level_rules rule \"" << rule.level << "\": invalid regex: " << e.what()
                   << '\n';
        continue;
      }
  ...
  if (line_formats.empty() || level_rules.empty()) {
    std::cerr << "[log_tail] patterns.json has no usable line_formats/level_rules -- keeping previous rules\n";
    return;
  }
  ...
  if (root.contains("tag_pattern") && root["tag_pattern"].is_string()) {
    try {
      ...
      std::cerr << "[log_tail] ignoring invalid tag_pattern: " << e.what() << '\n';
    }
  }
