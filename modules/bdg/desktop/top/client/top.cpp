...
/// @file top.cpp
/// @brief Client-side runner for the top embedded app.
///
/// The Top form (server-side) never samples anything itself --
...
/// how nano's reference client owns upload_file/download_file while the
/// server only manages tabs (see modules/bdg/desktop/nano/client/nano.cpp).
#include "modules/bdg/desktop/top/client/top.hpp"
#include "modules/bdg/desktop/top/client/process_control.hpp"
#include "modules/bdg/desktop/top/client/process_info.hpp"
...
// action-specific extra fields -- see top.hpp's doc comment on
// "on_process_action_requested"}) and reports the outcome back to the form
// via `report_action_result`, mirroring how mc's client reports
...
void run_top(wish_app_host& s) {
  auto proxy = std::make_shared<rmi::proxy::dynamic>(s.instantiate("wish"_key, "Top"_key).get());
  ...
  // mirroring nano's `nano` proxy (see its comment).
...
namespace {
struct top_app_registrar {
  top_app_registrar() {
    register_app({
        .name = "top",
        .organization = WISH_MODULE_BDG_DESKTOP_TOP_ORGANIZATION,
        .collection = WISH_MODULE_BDG_DESKTOP_TOP_COLLECTION,
...
const top_app_registrar top_app_registrar_instance;
} // namespace
