...
/// wish server's host -- see top.cpp in this directory, which
/// periodically pushes a sample into the server-side Top form
...
/// each `run_top` session owns exactly one instance and calls
