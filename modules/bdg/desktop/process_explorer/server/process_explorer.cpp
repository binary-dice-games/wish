...
// Confirm-kill dialog -- mirrors file_explorer's overwrite-confirmation
...
void process_explorer::on_init() {
  ...
  internal_root_key_ = next_available_key("__procexp_");
...
dynamic process_explorer::do_report_action_result(const dynamic& args) {
...
dynamic process_explorer::do_report_process_details(const dynamic& args) {
  int32_t pid = args.as<int32_t>("pid"_key);
...
void process_explorer::set_status(const std::string& text) {
  if (status_label_)
...
ui_element_ptr process_explorer::build_row_context_menu(row_entry& entry, int pid, const std::string& state, int32_t nice) {
...
void process_explorer::update_row_context_menu(row_entry& entry, const std::string& state, int32_t nice) {
...
void process_explorer::show_confirm_kill(int pid) {
  ...
  context& s = *lock;
  for (int i = 0;; ++i) {
    std::string candidate = "__procexp_confirm_" + std::to_string(i);
...
void process_explorer::request_close_confirm() {
  request_close_at(confirm_root_key_);
}

void process_explorer::remove_confirm_objects() {
...
void process_explorer::show_affinity_dialog(int pid) {
  ...
  context& s = *lock;
  for (int i = 0;; ++i) {
    std::string candidate = "__procexp_affinity_" + std::to_string(i);
    if (s.top_level_objects.find(key_t{candidate}) == s.top_level_objects.end()) {
...
void process_explorer::request_close_affinity() {
  request_close_at(affinity_root_key_);
}

void process_explorer::remove_affinity_objects() {
...
void process_explorer::show_properties_dialog(int pid) {
...

  {
    ...
    context& s = *lock;
    for (int i = 0;; ++i) {
      std::string candidate = "__procexp_properties_" + std::to_string(i);
      if (s.top_level_objects.find(key_t{candidate}) == s.top_level_objects.end()) {
...
void process_explorer::request_close_properties() {
  request_close_at(properties_root_key_);
}

void process_explorer::remove_properties_objects() {
...
void register_process_explorer() {
  ...
  proto->addMethod(
      "report_action_result"_key, bison::method{[](dynamic& self, const dynamic& args) -> dynamic {
        return static_cast<process_explorer&>(self).do_report_action_result(args);
      }});

  proto->addMethod(
      "report_process_details"_key, bison::method{[](dynamic& self, const dynamic& args) -> dynamic {
        return static_cast<process_explorer&>(self).do_report_process_details(args);
      }});
