...
/// @file bc.cpp
/// @brief Implementation of the bc form.
#include "bc.hpp"
...
void bc::handle_digit(const std::string& ch) {
  if (fresh_) {
    display_ = ch;
    fresh_ = false;
...
void bc::handle_operator(char op) {
...
void register_bc() {
  ...
  dynamic::addClass(
      "wish"_key, std::move(proto), key_t{0U}, dynamic::make_factory<bc>("wish"_key, "Bc"_key));
}
