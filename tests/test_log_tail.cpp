...
class LogTailLocalTest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(LogTailLocalTest, CanBeInstantiated) {
  auto obj = dynamic::instantiate("wish"_key, "LogTail"_key);
  auto* cls = obj.findField(dynamic::CLASS);
  ASSERT_NE(cls, nullptr);
  EXPECT_EQ(cls->as<bison::key_t>(), "LogTail"_key);
}

TEST_F(LogTailLocalTest, DefaultTitleIsLogTail) {
  auto obj = dynamic::instantiate("wish"_key, "LogTail"_key);
  auto* f = obj.findField("title"_key);
  ASSERT_NE(f, nullptr);
  EXPECT_TRUE(f->is<std::string>());
  EXPECT_EQ(f->as<std::string>(), "Log Tail");
}

// ── Session-capturing server, mirroring test_notepad.cpp's fixture ────────────
...
static std::string find_form_root(const wish::name_map& objects) {
  for (const auto& [k, _] : objects) {
    if (k.rfind("__log_tail_", 0) == 0 && k.find('.') == std::string::npos)
      return k;
  }
  return {};
}
...
// Dynamically-added TableRow/TabItem elements (see log_tail.cpp's append_row/
// ensure_tag_tab) are never registered in ui_objects by dot-path -- only the
// statically JSON-declared chrome is -- so, mirroring test_notepad.cpp's
...
class LogTailTest : public ::testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    client_->connect();
    proxy_.emplace(client_->instantiate("wish"_key, "LogTail"_key).get());
...
TEST_F(LogTailTest, LevelPrefixedLineIsClassifiedError) {
...
TEST_F(LogTailTest, TaggedLineCreatesTagTab) {
...
TEST_F(LogTailTest, BracketedLevelWordIsNotTreatedAsATag) {
  push_line("[ERROR] disk full", "app.log");
...
TEST_F(LogTailTest, StatusLabelTracksLineCount) {
  push_line("one", "a.log");
  push_line("two", "a.log");
...
TEST_F(LogTailTest, FilterAppliesProspectivelyOnly) {
...
TEST_F(LogTailTest, InvalidFilterRegexLeavesPreviousFilterActive) {
  set_filter("keep");
...
TEST_F(LogTailTest, ClearFilterButtonRestoresUnfilteredIngestion) {
  set_filter("nomatch");
...
TEST_F(LogTailTest, ClearAllButtonEmptiesAllTable) {
  push_line("one", "a.log");
  push_line("two", "a.log");
...
TEST_F(LogTailTest, WindowClosedEmitsClosed) {
