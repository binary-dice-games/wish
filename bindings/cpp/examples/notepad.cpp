...
//
// Notepad example using the header-only C++ wish client binding.
//
// Port of bindings/python/examples/notepad_example.py (itself a port of
// modules/notepad/client/notepad.cpp): the Notepad form (server-side) never
...
//
// Usage: notepad_cpp [--transport=tcp|pipe|term] [--host=HOST] [--port=PORT]
...
// Mirrors list_directory() in modules/notepad/client/notepad.cpp: the C ABI
...
// name and registers it as a new Notepad tab. Shared by the FileDialog-driven
// Open/New flows and by opening a file passed on the command line.
void upload_and_open(wish::client& client, wish::proxy& notepad, sandbox_files& files, const fs::path& local_path) {
  ...
  notepad.call("open_file"_key, params);
}
...
void browse_and_open(
  ...
  dlg->on_event(
      "on_open"_key,
      [&client, &notepad, &files, cur_dir, create_if_missing](wish::value payload) {
...
void run_notepad(wish::client& client, const std::optional<fs::path>& startup_path) {
  auto notepad = client.instantiate("Notepad"_key, "wish"_key);
  sandbox_files files;

  // "Open" clicked: the server has no view of the client's local files, so
  // it asks us to present our own picker.
  notepad.on_event("on_request_open"_key, [&](wish::value) {
  ...
  notepad.on_event("on_request_new"_key, [&](wish::value) {
  ...
  notepad.on_event("on_file_closed"_key, [&](wish::value payload) {
  ...
  notepad.on_event("on_file_saved"_key, [&](wish::value payload) {
    std::string path = payload.get_string("path"_key).value_or("");
    auto it = files.local_path_by_sandbox_name_.find(path);
  ...
  notepad.on_event("on_sync_requested"_key, [&](wish::value payload) {
  ...
  notepad.on_event("closed"_key, [&client](wish::value) { client.quit(); });
  ...
  if (startup_path) {
    if (fs::exists(*startup_path))
      ...
      std::cerr << "[notepad] no such file: " << startup_path->string() << "\n";
  }
