"""Nano example using the wish Python binding.

Port of modules/bdg/desktop/nano/client/nano.cpp: the nano form (server-side)
...
(see wish server --help):
    ...
    python bindings/python/examples/nano_example.py --transport=tcp --host=127.0.0.1 --port=7070 -- path/to/file

Run with:  python bindings/python/examples/nano_example.py [--transport=tcp|pipe|term] [--host=HOST] [--port=PORT] [--name=PATH] [--theme=dark|light|classic] [-- path/to/file]
"""
...
def list_directory(directory: Path) -> list:
    ...
    Mirrors list_directory() in modules/bdg/desktop/nano/client/nano.cpp.
    """
    files = []
...
def upload_and_open(client: Client, nano, files: SandboxFiles, local_path: Path) -> None:
    ...
    nano.open_file(path=sandbox_name, title=local_path.name)
...
def run_nano(client: Client, startup_path: Optional[Path]) -> None:
    nano = client.instantiate("Nano", "wish")
    files = SandboxFiles()
    ...
    nano.on_event("on_request_open", on_request_open)
    ...
    if startup_path is not None:
        if startup_path.exists():
            ...
            print(f"[nano] no such file: {startup_path}", file=sys.stderr)
