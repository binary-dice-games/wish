...
| `WISH_COLLECTION_BDG_DEV` | `OFF` | Include every module in `modules/bdg/dev/` (editor, docker, kubectl, dbg) — see below. |
...
| `WISH_MODULE_BDG_DEV_DBG` | `OFF` | Include the DebuggerFrontend form (server) and its self-registering reference client runner — a source-level debugger GUI attaching to a local process by PID (`wish client --run=dbg`). Selectable backend via `-- --backend <native\|python>`: `native` (default) is `win32_debug_backend` (Win32 debug API + DbgHelp) on Windows / `posix_debug_backend` (child `gdb --interpreter=mi` / `lldb-mi`) on Linux/macOS — needs `gdb`/`lldb-mi` at run time; `python` is `python_debug_backend`, driving `debugpy` over the Debug Adapter Protocol — needs `debugpy` at run time. |
...
collection or the individual option is turned off. The `bdg/dev` collection
below defaults `OFF`.
...
`.github/workflows/release-binaries.yml` builds the zip on Linux
(x86_64 + aarch64), macOS arm64, and native Windows (MSVC), and
`.github/workflows/release-deb.yml` builds the amd64 `.deb` — all attached
to the release. See [docs/publishing-binaries.md](publishing-binaries.md)
for the runbook.
