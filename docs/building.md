...
| `WISH_MODULE_BDG_DEV_DBG` | `OFF` | Include the DebuggerFrontend form (server) and its self-registering reference client runner — a source-level debugger GUI attaching to a local process by PID (`wish client --run=dbg`). Backend is `win32_debug_backend` (Win32 debug API + DbgHelp) on Windows, `posix_debug_backend` (drives a child `gdb --interpreter=mi` / `lldb-mi`) on Linux/macOS — the latter needs `gdb`/`lldb-mi` installed at run time. |
